Function Security-Filter([string]$gpo_nme, [System.Collections.ArrayList]$userarray) {
	Set-GPPermissions -Name $gpo_nme -PermissionLevel GpoApply -TargetName "Компьютеры домена" -TargetType Group -Confirm:$false| out-null
...
if (!$BatchUser) {
	Set-GPPermissions -Name $GPODisplayName -PermissionLevel GpoRead -TargetName "Прошедшие проверку" -TargetType Group -Replace -Confirm:$false| out-null
}
Set-GPPermissions -Name $GPODisplayName -PermissionLevel GpoEdit -TargetName "Прошедшие проверку" -TargetType Group -Confirm:$false| out-null
